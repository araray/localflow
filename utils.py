...
class OutputHandler:
    ...
    def __init__(self, config: OutputConfig):
        if not isinstance(config.file, (Path, type(None))):
            raise TypeError(
                f"`file` in OutputConfig must be a Path or None, got {type(config.file)}"
            )
        self.config = config
    ...
    def __enter__(self):
        ...
        if (
            self.config
            and self.config.file
            and self.config.mode in (OutputMode.FILE, OutputMode.BOTH)
        ):
            try:
                ...
                logging.debug(
                    f"Output file {self.config.file} created with mode '{mode}'."
                )
    ...
    def write(self, content: str):
        ...
        if self.config.stdout and self.config.mode in (
            OutputMode.STDOUT,
            OutputMode.BOTH,
        ):
