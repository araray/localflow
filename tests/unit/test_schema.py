...
@pytest.fixture
def example_workflow_file(
    temp_dir: Path, example_workflow_content: dict
...
def test_condition_parse():
    ...
    cond = Condition.parse({"if": "job_1 and job_2", "needs": ["job_1", "job_2"]})
...
def test_condition_evaluate():
    ...
    cond = Condition.parse({"if": "job_1 and not job_2", "needs": ["job_1", "job_2"]})
...
def test_job_from_dict():
    ...
    job_data = {
        ...
        "description": "Test job",
        "tags": ["test"],
        "condition": {"if": "dep_job", "needs": ["dep_job"]},
